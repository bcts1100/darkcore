...
var Transaction = darkcore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/dark.conf')));
var darkService = proxyquire('../../lib/services/darksagad', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultdarkConf = fs.readFileSync(path.resolve(__dirname, '../data/default.dark.conf'), 'utf8');

describe('dark Service', function() {
  ...
  var baseConfig = {
    node: {
      network: darkcore.Networks.testnet
    },
    spawn: {
      datadir: 'testdir',
      exec: 'testpath',
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var darksagad = new darkService(baseConfig);
      should.exist(darksagad);
    });
    it('will create an instance without `new`', function() {
      var darksagad = darkService(baseConfig);
      should.exist(darksagad);
    });
    it('will init caches', function() {
      var darksagad = new darkService(baseConfig);
      should.exist(darksagad.utxosCache);
      should.exist(darksagad.txidsCache);
      should.exist(darksagad.balanceCache);
      should.exist(darksagad.summaryCache);
      should.exist(darksagad.transactionDetailedCache);
      should.exist(darksagad.masternodeListCache);

      should.exist(darksagad.transactionCache);
      should.exist(darksagad.rawTransactionCache);
      should.exist(darksagad.blockCache);
      should.exist(darksagad.rawBlockCache);
      should.exist(darksagad.blockHeaderCache);
      should.exist(darksagad.zmqKnownTransactions);
      should.exist(darksagad.zmqKnownBlocks);
      should.exist(darksagad.lastTip);
      should.exist(darksagad.lastTipTimeout);
    });
    it('will init clients', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.nodes.should.deep.equal([]);
      darksagad.nodesIndex.should.equal(0);
      darksagad.nodes.push({client: sinon.stub()});
      should.exist(darksagad.client);
    });
    it('will set subscriptions', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var darksagad = new darkService(baseConfig);
      darksagad._initDefaults({transactionConcurrency: 10});
      darksagad.transactionConcurrency.should.equal(10);
      darksagad._initDefaults({});
      darksagad.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      darkService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var darksagad = new darkService(baseConfig);
      var methods = darksagad.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var darksagad = new darkService(baseConfig);
      var events = darksagad.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('darksagad/rawtransaction');
      events[0].scope.should.equal(darksagad);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('darksagad/transactionlock');
      events[1].scope.should.equal(darksagad);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('darksagad/hashblock');
      events[2].scope.should.equal(darksagad);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('darksagad/addresstxid');
      events[3].scope.should.equal(darksagad);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.subscribe = sinon.stub();
      darksagad.unsubscribe = sinon.stub();
      var events = darksagad.getPublishEvents();

      events[0].subscribe('test');
      darksagad.subscribe.args[0][0].should.equal('rawtransaction');
      darksagad.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      darksagad.unsubscribe.args[0][0].should.equal('rawtransaction');
      darksagad.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      darksagad.subscribe.args[1][0].should.equal('transactionlock');
      darksagad.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      darksagad.unsubscribe.args[1][0].should.equal('transactionlock');
      darksagad.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      darksagad.subscribe.args[2][0].should.equal('hashblock');
      darksagad.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      darksagad.unsubscribe.args[2][0].should.equal('hashblock');
      darksagad.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var darksagad = new darkService(baseConfig);
      var emitter = {};
      darksagad.subscribe('hashblock', emitter);
      darksagad.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      darksagad.subscribe('rawtransaction', emitter2);
      darksagad.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      darksagad.subscribe('hashblock', emitter1);
      darksagad.subscribe('hashblock', emitter2);
      darksagad.subscribe('hashblock', emitter3);
      darksagad.subscribe('hashblock', emitter4);
      darksagad.subscribe('hashblock', emitter5);
      darksagad.subscriptions.hashblock.length.should.equal(5);

      darksagad.unsubscribe('hashblock', emitter3);
      darksagad.subscriptions.hashblock.length.should.equal(4);
      darksagad.subscriptions.hashblock[0].should.equal(emitter1);
      darksagad.subscriptions.hashblock[1].should.equal(emitter2);
      darksagad.subscriptions.hashblock[2].should.equal(emitter4);
      darksagad.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var darksagad = new darkService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      darksagad.subscriptions.hashblock= [emitter1];
      darksagad.unsubscribe('hashblock', emitter3);
      darksagad.subscriptions.hashblock.length.should.equal(1);
      darksagad.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      darksagad.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(darksagad.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var darksagad = new darkService(baseConfig);
      var emitter = new EventEmitter();
      darksagad.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var darksagad = new darkService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      darksagad.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      darksagad.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      darksagad.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      darksagad.unsubscribeAddress(emitter1);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      darksagad.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      darksagad.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      darksagad.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      darksagad.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      darksagad.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(darksagad, 'unsubscribeAddressAll');
      darksagad.unsubscribeAddress(emitter1);
      darksagad.unsubscribeAddressAll.callCount.should.equal(1);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      darksagad.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      darksagad.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      darksagad.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      darksagad.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      darksagad.unsubscribeAddress(emitter1);
      darksagad.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      darksagad.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      darksagad.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(darksagad.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var darksagad = new darkService(baseConfig);
      var config = darksagad._getDefaultConfig();
      config.should.equal(defaultdarkConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a dark.conf file', function() {
      var Testdark = proxyquire('../../lib/services/darksagad', {
        fs: {
          readFileSync: readFileSync,
      ...
      var darksagad = new Testdark(baseConfig);
      darksagad.options.spawn.datadir = '/tmp/.dark';
      var node = {};
      darksagad._loadSpawnConfiguration(node);
      should.exist(darksagad.spawn.config);
      darksagad.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: darkcore.Networks.testnet,
          configPath: '/tmp/.darkcore/darkcore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var darksagad = new Testdark(config);
      darksagad.options.spawn.datadir = './data';
      var node = {};
      darksagad._loadSpawnConfiguration(node);
      darksagad.options.spawn.datadir.should.equal('/tmp/.darkcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var Testdark = proxyquire('../../lib/services/darksagad', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/baddark.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var darksagad = new Testdark(baseConfig);
      (function() {
        darksagad._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(darkcore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the dark.conf file', function() {
      var darksagad = new darkService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      darksagad._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        darksagad.transactionDetailedCache.set(keys[i], {});
        darksagad.utxosCache.set(keys[i], {});
        darksagad.txidsCache.set(keys[i], {});
        darksagad.balanceCache.set(keys[i], {});
        darksagad.summaryCache.set(keys[i], {});
      }
      darksagad._resetCaches();
      should.equal(darksagad.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(darksagad.utxosCache.get(keys[0]), undefined);
      should.equal(darksagad.txidsCache.get(keys[0]), undefined);
      should.equal(darksagad.balanceCache.get(keys[0]), undefined);
      should.equal(darksagad.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.tryAllInterval = 1;
      darksagad.nodes.push({
        client: {
      ...
      darksagad._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        darksagad.nodes[0].client.getInfo.callCount.should.equal(1);
        darksagad.nodes[1].client.getInfo.callCount.should.equal(1);
        darksagad.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert darksagad-rpc object into JavaScript error', function() {
      var darksagad = new darkService(baseConfig);
      var error = darksagad._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      darksagad.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      darksagad.nodes.push({
        client: {
      ...
      darksagad._initChain(function() {
        log.info.callCount.should.equal(1);
        darksagad.getRawBlock.callCount.should.equal(1);
        darksagad.getRawBlock.args[0][0].should.equal('genesishash');
        darksagad.height.should.equal(5000);
        darksagad.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      darksagad._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      darksagad.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      darksagad._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      darkcore.Networks.disableRegtest();
      baseConfig.node.network = darkcore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      var config = {
        node: {
          network: darkcore.Networks.livenet
        },
        spawn: {
          datadir: 'testdir',
          exec: 'testpath'
      ...
      var darksagad = new darkService(config);
      darksagad._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var darksagad = new darkService(config);
      darksagad._getDefaultConf().rpcport.should.equal(19998);
    });
    it('will get default rpc port for regtest', function() {
      darkcore.Networks.enableRegtest();
      var config = {
        node: {
          network: darkcore.Networks.testnet
        },
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var darksagad = new darkService(config);
      should.equal(darksagad._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var darksagad = new darkService(config);
      darksagad._getNetworkConfigPath().should.equal('testnet3/dark.conf');
    });
    it('will get default rpc port for regtest', function() {
      darkcore.Networks.enableRegtest();
      var config = {
      ...
      var darksagad = new darkService(config);
      darksagad._getNetworkConfigPath().should.equal('regtest/dark.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.node.network = darkcore.Networks.testnet;
      darksagad._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.node.network = darkcore.Networks.testnet;
      darkcore.Networks.enableRegtest();
      darksagad._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var darksagad = new darkService(baseConfig);
      darksagad.node.network = darkcore.Networks.livenet;
      darkcore.Networks.enableRegtest();
      should.equal(darksagad._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      darksagad._rapidProtectedUpdateTip = sinon.stub();
      darksagad.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      darksagad._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      darksagad._rapidProtectedUpdateTip = sinon.stub();
      darksagad._zmqBlockHandler(node, message);
      darksagad._rapidProtectedUpdateTip.callCount.should.equal(1);
      darksagad._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      darksagad._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      darksagad._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      darksagad.subscriptions.hashblock.push(emitter);
      emitter.on('darksagad/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        darksagad._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub();
      darksagad.on('error', function(err) {
      ...
      darksagad._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      darksagad.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      darksagad.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      darksagad.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub();
      darksagad._resetCaches = sinon.stub();
      darksagad.on('tip', function(height) {
        darksagad._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        darksagad.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      darksagad.on('tip', function() {
        darksagad.syncPercentage.callCount.should.equal(0);
        done();
      });
      darksagad._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using darkcore.Transaction', function() {
      var darksagad = new darkService(baseConfig);
      var wif = 'XGLgPK8gbmzU7jcbw34Pj55AXV7SmG6carKuiwtu4WtvTjyTbpwX';
      var privkey = darkcore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(darkcore.Networks.testnet);
      var outputAddress = darkcore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      var tx = darkcore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: darkcore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = darksagad._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(darkcore.Transaction.Output({
        script: darkcore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = darksagad._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var darksagad = new darkService(baseConfig);
      var tx = darkcore.Transaction();
      var address = darkcore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      tx.addOutput(darkcore.Transaction.Output({
        script: darkcore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(darkcore.Transaction.Output({
        script: darkcore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      darksagad._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      darksagad.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('darksagad/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      darksagad._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      darksagad.subscriptions.rawtransaction.push(emitter);
      emitter.on('darksagad/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      darksagad._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      darksagad.subscriptions.rawtransaction.push(emitter);
      emitter.on('darksagad/rawtransaction', function() {
        done();
      });
      var node = {};
      darksagad._zmqTransactionHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var darksagad = new darkService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      darksagad.on('tx', function(buffer) {
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      darksagad.subscriptions.transactionlock.push(emitter);
      emitter.on('darksagad/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      darksagad._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var darksagad = new darkService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      darksagad.subscriptions.transactionlock.push(emitter);
      emitter.on('darksagad/transactionlock', function() {
        done();
      });
      var node = {};
      darksagad._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var darksagad = new darkService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      darksagad.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._updateTip = sinon.stub();
      darksagad._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      darksagad.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      darksagad._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        darksagad._updateTip.callCount.should.equal(11);
        darksagad._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      darksagad._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        darksagad._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      darksagad._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        darksagad._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      darksagad._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        darksagad._zmqBlockHandler.callCount.should.equal(0);
        darksagad._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var darksagad = new darkService(baseConfig);
      var node = {};
      darksagad._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      darksagad._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      darksagad._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      darksagad._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      darksagad.on('tip', function(height) {
        ...
        darksagad.height.should.equal(100);
        done();
      });
      darksagad._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestdarkService = proxyquire('../../lib/services/darksagad', {
        fs: {
          readFile: readFile
        }
      });
      var darksagad = new TestdarkService(baseConfig);
      darksagad.spawnStopTime = 1;
      darksagad._process = {};
      darksagad._process.kill = sinon.stub();
      darksagad._stopSpawneddark(function(err) {
        if (err) {
          return done(err);
        }
        darksagad._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      darksagad._process.kill = sinon.stub().throws(error2);
      darksagad._stopSpawneddark(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      darksagad._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawneddark', function() {
      var darksagad = new darkService(baseConfig);
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad._stopSpawneddark = sinon.stub().callsArgWith(0, new Error('test'));
      darksagad._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var darksagad = new TestdarkService(config);
      darksagad.spawn = {};
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad._stopSpawneddark = sinon.stub().callsArgWith(0, null);
      darksagad.node.stopping = true;
      darksagad._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad.spawn = {};
      darksagad.spawn.exec = 'testexec';
      darksagad.spawn.configPath = 'testdir/dark.conf';
      darksagad.spawn.datadir = 'testdir';
      darksagad.spawn.config = {};
      darksagad.spawn.config.rpcport = 20001;
      darksagad.spawn.config.rpcuser = 'dark';
      darksagad.spawn.config.rpcpassword = 'password';
      darksagad.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      darksagad.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      darksagad._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      darksagad._initZmqSubSocket = sinon.stub();
      darksagad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      darksagad._checkReindex = sinon.stub().callsArgWith(1, null);
      darksagad._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/dark.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        darksagad._loadTipFromNode.callCount.should.equal(1);
        darksagad._initZmqSubSocket.callCount.should.equal(1);
        should.exist(darksagad._initZmqSubSocket.args[0][0].client);
        darksagad._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        darksagad._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(darksagad._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn darksagad spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestdarkService = proxyquire('../../lib/services/darksagad', {
        fs: {
      ...
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad.spawn = {};
      darksagad.spawn.exec = 'darksagad';
      darksagad.spawn.datadir = '/tmp/dark';
      darksagad.spawn.configPath = '/tmp/dark/dark.conf';
      darksagad.spawn.config = {};
      darksagad.spawnRestartTime = 1;
      darksagad._loadTipFromNode = sinon.stub().callsArg(1);
      darksagad._initZmqSubSocket = sinon.stub();
      darksagad._checkReindex = sinon.stub().callsArg(1);
      darksagad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      darksagad._stopSpawneddark = sinon.stub().callsArg(0);
      sinon.spy(darksagad, '_spawnChildProcess');
      darksagad._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            darksagad._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      darksagad._spawnChildProcess(function(err) {
        ...
        darksagad._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        darksagad.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn darksagad spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestdarkService = proxyquire('../../lib/services/darksagad', {
        fs: {
      ...
      darksagad._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            darksagad._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      var darksagad = new TestdarkService(baseConfig);
      darksagad.startRetryInterval = 1;
      darksagad._loadSpawnConfiguration = sinon.stub();
      darksagad.spawn = {};
      darksagad.spawn.exec = 'testexec';
      darksagad.spawn.configPath = 'testdir/dark.conf';
      ...
      darksagad.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      darksagad.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      darksagad._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      darksagad._spawnChildProcess(function(err) {
        darksagad._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      darksagad._initZmqSubSocket = sinon.stub();
      darksagad._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      darksagad._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var darksagad = new darkService(config);
      darksagad.node.stopping = true;
      darksagad.startRetryInterval = 100;
      darksagad._loadTipFromNode = sinon.stub();
      darksagad._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        darksagad._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      darksagad.startRetryInterval = 1;
      var config = {};
      darksagad._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        darksagad._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      darksagad._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      darksagad._connectProcess(config, function(err, node) {
        should.not.exist(err);
        darksagad._loadTipFromNode.callCount.should.equal(1);
        darksagad._initZmqSubSocket.callCount.should.equal(1);
        darksagad._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.options = {};
      darksagad.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/dark configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      darksagad.options = {
        connect: [
          {}
        ]
      ...
      darksagad.start(function(err) {
        darksagad._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var darksagad = new darkService(baseConfig);
      var node = {};
      darksagad._initChain = sinon.stub().callsArg(0);
      darksagad._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      darksagad.options = {
        spawn: {}
      };
      darksagad.start(function(err) {
        should.not.exist(err);
        darksagad.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      darksagad._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      darksagad._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      darksagad.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    it('will give error from syncPercentage', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      darksagad.isSynced(function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      darksagad.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      darksagad.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      darksagad.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      darksagad.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      darksagad.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var darksagad = new darkService(baseConfig);
      var args = darksagad._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var darksagad = new darkService(baseConfig);
      var args = darksagad._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      darksagad.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      darksagad.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        darksagad.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      darksagad.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      darksagad.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = darksagad._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = darksagad._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = darksagad._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      darksagad._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        darksagad._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      darksagad.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      darksagad.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      darksagad.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      darksagad.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        darksagad.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var darksagad = new darkService(baseConfig);
      darksagad.height = 10;
      var confirmations = darksagad._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var darksagad = new darkService(baseConfig);
      var result = {};
      darksagad._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var darksagad = new darkService(baseConfig);
      var result = {};
      darksagad._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var darksagad = new darkService(baseConfig);
      var result = {};
      darksagad._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var darksagad = new darkService(baseConfig);
      var result = {};
      darksagad._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = darksagad._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var darksagad = new darkService(baseConfig);
      darksagad.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      darksagad.height = 300;
      var addresses = {};
      darksagad._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      darksagad._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var darksagad = new darkService(baseConfig);
      darksagad.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      darksagad._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from darkcore addresses', function() {
      var addresses = [
        darkcore.Address('XjxDQFjTNEP9dcrJhBLvy5i1Dobz4x1LJN'),
        darkcore.Address('7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'),
      ];
      var darksagad = new darkService(baseConfig);
      var strings = darksagad._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var darksagad = new darkService(baseConfig);
      (function() {
        darksagad._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      darksagad.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      darksagad._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      darksagad.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var darksagad = new darkService(baseConfig);
      darksagad.maxAddressesQuery = 100;
      darksagad.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      darksagad.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      darksagad.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      darksagad.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      darksagad.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      darksagad.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      darksagad.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(darksagad, '_paginate');
      darksagad.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      darksagad.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      darksagad.getAddressSummary(address, options, function(err, summary) {
        darksagad._paginate.callCount.should.equal(1);
        darksagad._paginate.args[0][1].should.equal(0);
        darksagad._paginate.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      darksagad.getAddressSummary(address, options, function(err, summary) {
        ...
        darksagad.getAddressTxids.callCount.should.equal(1);
        darksagad.getAddressBalance.callCount.should.equal(1);
        darksagad.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          darksagad.getAddressTxids.callCount.should.equal(1);
          darksagad.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      darksagad.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginate', function(done) {
      ...
      darksagad._paginate = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      darksagad.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      darksagad.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      darksagad.nodesIndex = 2;
      darksagad.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      darksagad.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      darksagad.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as darkcore object from height', function(done) {
      var darksagad = new darkService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      darksagad.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(darkcore.Block);
        done();
      });
    });
    it('will getblock as darkcore object', function(done) {
      var darksagad = new darkService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      darksagad.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
    ...
    it('will get block from cache', function(done) {
      ...
      darksagad.getBlock(hash, function(err, block) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        getBlock.callCount.should.equal(1);
        block.should.be.instanceof(darkcore.Block);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      darksagad.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      darksagad.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      darksagad.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      darksagad.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      darksagad.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      darksagad.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      darksagad.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#getBlockHeaders', function(){
      ...
      it('will gave error from getBlockHash', function(){
          ...
          darksagad.getBlockHeaders(10, function(err) {
              err.should.be.instanceof(Error);
          });
      });
      it('it will give rpc error from client getblockheaders', function() {
          ...
          darksagad.getBlockHeaders(blockhash, function(err){
              err.should.be.instanceof(Error);
          });
      });
      it("will get an array of block headers", function(){
          ...
          darksagad.getBlockHeaders(_blockHash, function(err, blockHeader){
              should.not.exist(err);
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      darksagad._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      darksagad._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      darksagad._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      darksagad.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      darksagad.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      darksagad.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = darkcore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      darksagad.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      darksagad.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      darksagad.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = darkcore.Transaction();
      (function() {
        darksagad.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      darksagad.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      darksagad.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(darkcore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      darksagad.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      darksagad.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      darksagad.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      darksagad.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.getInfo(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      darksagad.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        darksagad.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      var gobject = sinon.stub().callsArgWith(2, null, {
        result: {
          "DataHex": "",
          "DataString": "[[\"proposal\",{\"end_epoch\":1483855193,\"name\":\"7ea9acff5ae83d8c9e217d30a2b4d10ef8f171f8\",\"payment_address\":\"yZ7DYkDHCHfH1dw7rKdYaKcVykZmunbqNL\",\"payment_amount\":84,\"start_epoch\":1483765282,\"type\":1,\"url\":\"https://www.dark.org\"}]]",
          "Hash": "4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00",
      ...
      darksagad.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
        ...
        should.equal(DataObject.url, 'https://www.dark.org');
        done();
      });
    });
	...
	describe('#sporksList', function(){
		it('will call client sporks and give result', function(done){
			...
			darksagad.getSpork(function(err, SporkList) {
				if (err) {
					return done(err);
				}
  ...
  describe('#getMNList', function(){
    it('will call client masternode list and give result', function(done){
	    var darksagad = new darkService(baseConfig);
	    darksagad.isSynced = function(callback) { return callback(null, true) };
	    darksagad.nodes.push({
		    client: {
	    ...
	    darksagad.getMNList(function(err, MNList) {
		    if (err) {
			    return done(err);
		    }
    ...
    it('will return error if one of nodes not synced yet', function(done){
      var darksagad = new darkService(baseConfig);
      darksagad.isSynced = function(callback) { return callback(null, false) };
      darksagad.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
    ...
    it('will return error if checking synced state of nodes failed', function(done){
      var darksagad = new darkService(baseConfig);
      darksagad.isSynced = function(callback) { return callback(new Error('Failed')) };
      darksagad.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      darksagad.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      darksagad.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.spawn = {};
      darksagad.spawn.process = new EventEmitter();
      darksagad.spawn.process.kill = sinon.stub();
      darksagad.stop(done);
      darksagad.spawn.process.kill.callCount.should.equal(1);
      darksagad.spawn.process.kill.args[0][0].should.equal('SIGINT');
      darksagad.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      darksagad.spawn.process.kill.callCount.should.equal(1);
      darksagad.spawn.process.kill.args[0][0].should.equal('SIGINT');
      darksagad.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var darksagad = new darkService(baseConfig);
      darksagad.shutdownTimeout = 300;
      darksagad.spawn = {};
      darksagad.spawn.process = new EventEmitter();
      darksagad.spawn.process.kill = sinon.stub();
      darksagad.stop(function(err) {
