...
describe('Dark Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.darkcore/data/darksagad')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/dark.conf'),
      rpcuser: 'dark',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/dark.conf'),
      rpcuser: 'dark',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/dark.conf'),
      rpcuser: 'dark',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 darksagad daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three Darksagad daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = Darkcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.darksagad.once('tip', function(height) {
