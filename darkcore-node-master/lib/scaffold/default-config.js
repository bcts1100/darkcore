...
/**
 * Will return the path and default darkcore-node configuration. It will search for the
 * configuration file in the "~/.darkcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.darkcore');
  var defaultConfigFile = path.resolve(defaultPath, './darkcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['darksagad', 'web'];
