...
/**
 * Will remove a service from darkcore-node.json
...
/**
 * Will remove the Node.js service and from the darkcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The darkcore-node configuration directory
...
function remove(options, done) {
  ...
  var darkcoreConfigPath = path.resolve(configPath, 'darkcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(darkcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a darkcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to darkcore-node.json
        removeConfig(darkcoreConfigPath, service, next);
      });
    }, done
  );
}
