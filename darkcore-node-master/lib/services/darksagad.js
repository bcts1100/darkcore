...
var Proposal = darkcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to Darksagad in Node.js. Manages starting and
 * stopping Darksagad as a child process for application support, as well as connecting
 * to multiple Darksagad processes for server infrastructure. Results are cached in an
...
util.inherits(Dark, Service);

Dark.dependencies = [];

Dark.DEFAULT_MAX_TXIDS = 1000;
Dark.DEFAULT_MAX_HISTORY = 50;
Dark.DEFAULT_MAX_UTXO = 1000;
Dark.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Dark.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Dark.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Dark.DEFAULT_SPAWN_RESTART_TIME = 5000;
Dark.DEFAULT_SPAWN_STOP_TIME = 10000;
Dark.DEFAULT_TRY_ALL_INTERVAL = 1000;
Dark.DEFAULT_REINDEX_INTERVAL = 10000;
Dark.DEFAULT_START_RETRY_INTERVAL = 5000;
Dark.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Dark.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Dark.DEFAULT_INSTANTSEND_FEE = 10000;
Dark.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Dark.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Dark.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Dark.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Dark.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Dark.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Dark.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Dark.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Dark.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Dark.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Dark.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Dark.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Dark.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Dark.prototype._initCaches = function() {
...
Dark.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Dark.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Dark.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'Darksagad/transactionlock',
      scope: this,
...
Dark.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'Darksagad/' + name, 'total:', this.subscriptions[name].length);
};

Dark.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'Darksagad/' + name, 'total:', this.subscriptions[name].length);
};

Dark.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (Darkcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'Darksagad/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Dark.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'Darksagad/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Dark.prototype.unsubscribeAddressAll = function(emitter) {
...
Dark.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Dark.DEFAULT_CONFIG_SETTINGS;
...
Dark.prototype._parseDarkConf = function(configPath) {
  var options = {};
...
Dark.prototype._expandRelativeDatadir = function() {
...
Dark.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in Darksagad config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in Darksagad config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in Darksagad config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './Dark.conf');

  log.info('Using Dark config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseDarkConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseDarkConf(networkConfigPath));
  }
...
Dark.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of Darkcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of Darkcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of Darkcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in Dark.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that Darksagad is undergoing a reindex. ' +
             ...
             'of Darkcore-node services will start.');
    node._reindex = true;
  }
};

Dark.prototype._resetCaches = function() {
 ...
 * @private
 */
Dark.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Dark.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Dark.prototype._initChain = function(callback) {
  var self = this;
...
Dark.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === Darkcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Dark.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === Darkcore.Networks.testnet) {
    networkPath = 'testnet3/Dark.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/Dark.conf';
    }
  }
  return networkPath;
};

Dark.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === Darkcore.Networks.testnet) {
...
Dark.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('Darksagad/hashblock', message.toString('hex'));
    }
  }

};

Dark.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Dark.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Dark Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Dark.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Dark.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('Darksagad/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Dark.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = Darkcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('Darksagad/rawtransaction', message.toString('hex'));
    }

    var tx = Darkcore.Transaction();
    tx.fromString(message);
    var txid = Darkcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Dark.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = Darkcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('Darksagad/transactionlock', message.toString('hex'));
    }

  }
};

Dark.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Dark.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Dark.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Dark.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Dark.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Dark Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Dark.DEFAULT_REINDEX_INTERVAL);

};

Dark.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Dark.prototype._stopSpawnedDark = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/Darksagad.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned Dark process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean Dark process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Dark.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedDark(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting Dark process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Dark process unexpectedly exited with code:', code);
        log.warn('Restarting Dark child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Dark process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn Darksagad.'));
      }
...
Dark.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to Darksagad.'));
    }
 ...
 */
Dark.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Dark configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Dark.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Dark.prototype.syncPercentage = function(callback) {
  var self = this;
...
Dark.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, Darkcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Dark.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = Darkcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Dark.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Dark.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Dark.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Dark.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in Darkcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Dark.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Dark.prototype._getConfirmationsDetail = function(transaction) {
...
Dark.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Dark.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Dark.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A Dark transaction id
 * @param {Function} callback
 */
Dark.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Dark.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof Darkcore.Address) {
...
Dark.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Dark.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Dark.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Dark.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Dark.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Darkcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Dark.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = Darkcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Dark.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Dark.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Dark.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Dark.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Dark.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Darkcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Dark.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Dark.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Dark.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Dark.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Dark.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Dark.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Dark.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Dark.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Dark.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Dark.prototype.govCount = function (callback) {
  var self = this;
...
Dark.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Dark.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Dark.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Dark.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Dark.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.getInfo = function(callback) {
  var self = this;
...
Dark.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Dark.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('Darksagad spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('Darksagad process did not exit'));
      }
